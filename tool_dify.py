...
class DifyDatasetClient:
    ...
    def upload_document_by_file(self, dataset_id: str, file_path: Path) -> Dict:
        ...
        print(
            f"配置中的父级分隔符: '{self.config['PARENT_SEPARATOR']}' (类型: {type(self.config['PARENT_SEPARATOR'])})")
        print(f"配置中的子级分隔符: '{self.config['CHILD_SEPARATOR']}' (类型: {type(self.config['CHILD_SEPARATOR'])})")
        print(
            f"配置中的父级最大字符: {self.config['PARENT_MAX_CHARS']} (类型: {type(self.config['PARENT_MAX_CHARS'])})")
        ...
        print(f"\n处理规则详情:")
        print(
            f"  - 父级分隔符: '{process_rule['rules']['segmentation']['separator']}' (类型: {type(process_rule['rules']['segmentation']['separator'])})")
        print(
            f"  - 父级最大字符: {process_rule['rules']['segmentation']['max_tokens']} (类型: {type(process_rule['rules']['segmentation']['max_tokens'])})")
        print(
            f"  - 子级分隔符: '{process_rule['rules']['subchunk_segmentation']['separator']}' (类型: {type(process_rule['rules']['subchunk_segmentation']['separator'])})")
        print(
            f"  - 子级最大字符: {process_rule['rules']['subchunk_segmentation']['max_tokens']} (类型: {type(process_rule['rules']['subchunk_segmentation']['max_tokens'])})")
